...
from app.db.schemas.journal import DraftCreate, Journal, JournalCreate, JournalSectionCreate
...
mood_classifier = pipeline("text-classification", model="SamLowe/roberta-base-go_emotions", top_k=None, truncation=True)
...
def generate_embedding(text: str) -> list:
    ...
    avg_embedding = [sum(col) / len(col) for col in zip(*embeddings)]  # Average to 384 dimensions
...
def analyze_mood(text: str) -> dict:
    ...
    moods = {result["label"]: result["score"] for result in mood_list}  # Convert to dict
...
def submit_draft():
    ...
    if not keys:
        raise APIException(
            status_code=400,
            detail="NO keys in Redis",
            message="No Drafts Present"
        )
    for key in keys:
...
def insert_journal_section(section_data: dict, db):
    try:
        response = db.table("journal_sections").insert(jsonable_encoder(section_data)).execute()
        if not response.data:
...
def process_draft_chunk(chunk: str, section_number: int, user_id: str, date: str, embedding_model, text_splitter, mood_classifier):
...
def aggregate_journal(chunks: List[str], user_id: str, date: str, embedding_model, text_splitter, mood_classifier):
    ...
    journal_data = {
        ...
        "embedding": generate_embedding(aggregated_content, embedding_model, text_splitter)  # 384 dims
    }
