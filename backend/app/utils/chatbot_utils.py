        ...
        "function_declarations": [
            {
                ...
                "parameters": {
                    "type": "object",
                    "properties": {
                        "user_id": {"type": "string", "description": "The ID of the user querying their journals."},
                        "start_date": {"type": "string", "description": "Start date in ISO format (e.g., '2025-04-06') or relative term (e.g., 'yesterday')."},
                        "end_date": {"type": "string", "description": "End date in ISO format (e.g., '2025-04-06') or relative term (e.g., 'today')."}
                    },
                    "required": ["user_id"]
                }
            },
            {
                ...
                "parameters": {
                    "type": "object",
                    "properties": {
                        "title": {"type": "string", "description": "The title to search for (e.g., 'Cricket Match')."},
                        "journal_ids": {
                            "type": "array",
                            "items": {"type": "string"},
            ...
            },
            {
                ...
                "parameters": {
                    "type": "object",
                    "properties": {
                        "topic": {"type": "string", "description": "The main subject or topic to search for (e.g., 'work', 'cricket')."},
                        "journal_ids": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "List of journal IDs to filter."
...
def query_function(user_id: str, user_query: str, filter_parameters)  :
    ...
    try:
        ...
        for part in function_calls:
            if part.function_call:
                ...
                if function_name == "get_journals_by_date" and not date_called:
                    start_date = args.get("start_date", filter_parameters.get("date_range", {}).get("start") if filter_parameters else None)
                    end_date = args.get("end_date", filter_parameters.get("date_range", {}).get("end") if filter_parameters else None)
                    if start_date and not end_date:
                        end_date = start_date  # Handle single-day queries if end is missing
                    journals = get_journals_by_date(user_id, start_date, end_date)
                    current_data = journals
                    date_called = True
                elif date_called:
                    if function_name == "filter_by_title":
                        title = args.get("title", filter_parameters.get("title", "") if filter_parameters else (user_query.split()[-1] if "titled" in user_query.lower() else None))
                        journal_ids = [j["id"] for j in current_data] if current_data else []
                        if title:
                            title_result = filter_by_title(title, journal_ids)

                    if function_name == "filter_by_embeddings":
                        topic = args.get("topic", user_query.split()[-1] if any(word in user_query.lower() for word in ["about", "on"]) else user_query)
                        journal_ids = [j["id"] for j in current_data] if current_data else []
        ...
        if not current_data:
            response_text = f"Sorry, I couldn’t find any journals matching '{user_query}'."
            ...
            response_text = f"Here’s the journal titled '{title}' you wrote on {date}: {text}"
        else:
